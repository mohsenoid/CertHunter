...
The status marker SHALL be exactly one of three values: `VALID` for `CertificateValidity.Valid`, `EXPIRES IN <daysLeft> DAYS` for `CertificateValidity.ExpiringSoon(daysLeft)`, and `EXPIRED` for `CertificateValidity.Expired`. All three values SHALL be sourced from string resources so each locale can adapt them.
...
- **AND** the body contains the SHA-256, SHA-1, owner, issuer, serial, `validFrom`–`validUntil` range, and a `VALID` status line
...
- **THEN** the shared text's status line for that signer reads `EXPIRED`
...
- **AND** the user taps share
- **THEN** the shared text's status line for that signer reads `EXPIRES IN <daysLeft> DAYS`
